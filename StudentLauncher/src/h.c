...
void deschide_mod_grafic_student(void) {
    ...
    if (mod_comparare == 2) {
        for (i = 0; i < nr_studenti && nr_fisiere_lansare < 5; i++) {
            student_comparat = &studenti[i];
        ...
        for (i = 0; i < numar_comparatii; i++) {
            ...
            student_comparat = cauta_student_dupa_cod(cod_comparat);
    ...
    for (i = 0; i < nr_fisiere_lansare; i++) {
        ...
        if (scris < 0 || (size_t)scris >= sizeof(parametri_lansare) - len_parametri) {
            printf("Lista fisierelor pentru comparatie este prea lunga pentru lansare.\n");
            return;
        }
